...
The function to import or create rasters is `rast`. Note that when you start to use multiple packages in scripts, it can help to keep track of where functions are coming from by using the package name followed by two colons before the function name, e.g. `terra::rast()`. However, it's optional as long as you load the library first (`library(terra)`).
...
As we know that the `sp_data` are in the same order as `sp_locations` we can join the two data sets together with `cbind` (c for column bind). However, it is **much** safer, and **better practice** to use lookup functions (e.g. `dplyr::left_join`) for this with a common id column.
...
Now continue in a project in R studio, download the same data as above from the KLE, copy the code in, make comments, and then complete the code to find the mean elevation (and standard deviation) for both species of hare.
