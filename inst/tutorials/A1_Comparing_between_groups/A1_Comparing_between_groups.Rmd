...
-2.3 Now plot the data. First plot just the weight column and check for any outliers (this kind of plot is sometimes known as an index plot).
...
-6.2  Make a plot of the data for your report
...
You are working on the conservation of a threatened lizard in the Alps, in Europe. There are believed to be two subspecies, *A. variatus* and *A. minimalis*. *A. variatus* is much more common than *A. minimalis*, a fact which might influence potential conservation strategies. You want to investigate ecological differences between the subspecies. Is there a difference in altitudinal distribution between the two subspecies?  

Download `subspecies_elevation_data.csv` from the Data folder on this week's KLE page, follow the analysis protocol and answer the question! The data contains the elevation of different occurrence records for the two subspecies.
...
```{r iris_summary-solution, exercise.reveal_solution = TRUE}
...
Remember that if the `p-value` is < 0.05, then we can reject the Null Hypothesis. Here is the code for the shapiro test. First we will test all the data together:
...
The other condition is that the variance of each group is also similar. We can use the Bartlett Test to evaluate whether the variance of the sepal widths for each species is similar (the boxplot you made can also help evaluate this). Here is the code for the Bartlett Homogeneity of Variance test on the sepal width data:
...
We now conclude that we can use the ANOVA test, as our data is normally distributed and there are no significant differences between the variances of each group.

5. At last we can perform the ANOVA test. Here is the code for an ANOVA test. Note that we have assigned the result of the test to an object called `aov1`.
...
```{r iris_post_summary, exercise = TRUE,exercise.setup = "iris_post"}
...
You have measured invertebrate species richness across four different habitats (broadleaf woodland, conifer woodland, grassland, park) from 20 samples within each habitat. You are interested in whether there is a significant difference in species richness across the habitats, and if so, which habitats are different.
