...
Regression analysis with multiple predictors (or independent variables) follows the same analysis steps as before and results in similar outputs. One difference is how you can visualise your analysis, both during the exploratory stage, as well as when presenting your results. With any more than two variables, it's difficult to visualise all the predictors with a scatterplot (but you can use pairs of variables in multiple plots). 

Another difference is in the output. You will see a coefficient, standard error and p-value for each predictor you include in the model. First, check the overall result of the model, then you can look at how each of the individual predictors affects the response variable.
...
As before, check your data have imported correctly by using the `head()` and `str()` functions. Do this below, after running the code to import the data.
...
Use the summary function on the whole data.frame to see the range and median values of each numeric variable. This is also a useful check on your data. Some data should be within certain values (e.g. percentages should be between 0 and 100).
...
-4 *Run the test*  
The code for a regression with multiple predictors is very similar to a regression with just one predictor. We add the predictors to our *formula* argument:
...
Look at the 12th value of the *scaled* `forest$distance` vector. It's value is almost -1. This means its real value is almost one standard deviation below the mean. Use R to calculate this value in the code box below.
...
Remember that the scaled value is the value minus the mean, divided by the sd.
```{r scaling5, exercise = TRUE}
# e.g. for the 12th value of forest$distance:

(forest$distance[12] - mean(forest$distance)) /sd(forest$distance)
