...
plot(st_geometry(roads), col = "black")
...
```{r print_sf, exercise = TRUE, exercise.setup = "import_sf"}

```

```{r print_sf-solution, exercise.reveal_solution = TRUE}
...
```{r import_csv-solution, exercise.reveal_solution = TRUE}
...
```{r convert_csv_sf, exercise = TRUE, exercise.setup = "import_csv"}
...
```{r plot_csv_sf, exercise = TRUE, exercise.setup = "convert_csv_sf"}
...
```{r chull2_plot, exercise = TRUE, exercise.setup = "chull2"}
...
```{r intct_area, exercise = TRUE, exercise.setup = "intct1"}
...
```{r clip_extract, exercise = TRUE, exercise.setup = "clip1"}
...
2. Import the veteran tree points **vtrees.shp**, CEH landcover polygons **ceh_lc23_ds.shp** and road lines **OS_vml_road_centreline.shp** shapefiles into R as individual sf objects.  
...
13. Combine (use `c()`) the random points from (11) and (12) into a new object, create an sf `data.frame` with a column for the type of point (veteran tree sample / non veteran sample) and then save to a shapefile and/or R object on disk.
