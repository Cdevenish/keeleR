...
set.seed(999)
mp2 <- data.frame(x = as.integer(runif(15, 1, 14)),
                  y = as.integer(runif(15, 1, 14)))
...
Here is an sf `data.frame` for a point file. It contains four columns of attribute data (id, girth, species, status) and the geometry column with the point coordinates.
...
******
**R code reminder:** You can subset a `data.frame` by using the square brackets [,] directly after the `data.frame` name. Before the comma: select rows with row numbers `c(1,4,6,10)` or a logical expression `vtrees$species == "oak"`; after the comma: select columns with column names `c("species", "id")`, numbers `1:3` or a logical expression.
******
...
We can add other layers to a plot by plotting each in turn. In the first plot, you need to use `reset = FALSE` to allow a second plot on top, and then we use `add = TRUE` in subsequent calls to plot to add extra layers.

Here we plot a simple polygon of Keele campus, and then add the veteran tree locations.

```{r sf_plot_add, exercise = TRUE}

#plot(keele_campus, reset = FALSE)
plot(vtrees[,"species"], add =T)

```

There are other ways to plot spatial information, especially for cartography (with the `tmap` package. We will look at these later.

#### Importing spatial data into R

To import data from a shapefile (or other spat
ial format, e.g. .kml, .gpx), use the `st_read()` function. For most formats, you can simply use the path to your file. Remember, for shapefiles, these are made up multiple components, they all need to be in the same folder, but you only need to include the .shp component in your `st_read()` function.
...
```{r import_sf, exercise = T}

keele_paths <- st_read("data/keele_paths.shp")

```

Print the `keele_paths` object and look at the structure of the data frame, the coordinate system and the extent.

##### Transforming between Coordinate Reference Systems

As the veteran trees shapefile is in WGS 1984 geographic coordinates. It would be more useful to have this as British National Grid, so that it is compatible with the other spatial data we are going to use later. The `st_transform()` function to convert between CRS. 
...
Note that we have to place the original sf object first (vtrees), then the new CRS. The format for the coordinate reference system follows the [EPSG standard](). As you can see above, the British National Grid has the [code 27700]().  
...
Here we have three lines in an sf data frame. The object is `lns`. We can print the object to see the geometry type, the attribute columns (here, just `id`) and the bounding box of the data frame.

```{r lns1, exercise = TRUE}

lns

```

Plot the lines! Use `lwd = 2` inside the `plot()` function to increase the width of the lines (only on the plot).
...
What's the area of the inner buffers? 

```{r inner_area, exercise = TRUE, exercise.setup = "neg_buffer"}
...
Intersections are fundamental geometrical operations. For example, we can use them to calculate area of overlap in geometries.

Look back at the plot of the circles and other polygons. Note the shapes of the overlaps.
...
```{r clip2, exercise = TRUE, exercise.setup = "clip1"}
plot(rnd_pts, pch = 16)
plot(st_geometry(circle3), add= TRUE)
```

In the `st_intersects()` function, the first argument is the shape (or geometry) that we are asking that "yes / no" question about. For example, do the points (1^st^ argument) lie inside the circle (2^nd^ argument)?

```{r, clip2-solution}

pts_intersect <- st_intersects(rnd_pts, circle3)

plot(st_geometry(crcls[crcls$id == "circle3",]), add = T)
```
